...
- **新 Worker**：第一次部署时还没有 Bindings，wrangler 会自动建一套 D1 和 R2 并绑上。
...
仓库里的 `npm run build` 就是 OpenNext 构建。默认的 `wrangler deploy` 没有绑定就自动建 D1/R2，有绑定就沿用，并种齐 Variables and Secrets 字段名。
...
打开站点首页，点「管理后台」（或直接访问 `/admin`）。用第 4 步设的账号登录。
