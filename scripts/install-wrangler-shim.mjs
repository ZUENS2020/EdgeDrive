#!/usr/bin/env node
/**
 * Point wrangler invocations at scripts/wrangler-shim.mjs so Cloudflare's
 * default `npx wrangler deploy` / `npm exec wrangler` pick up deploy hooks.
 *
 * Only rewrite node_modules/wrangler/bin/wrangler.js. npm's
 * node_modules/.bin/wrangler is a symlink to that file — writing both
 * overwrites the same path with the wrong relative lookup.
 */
import {
  chmodSync,
  existsSync,
